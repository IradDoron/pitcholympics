...
import { getDictionaryClient, handleEndLevel } from '@utils';
...
const Page = ({ params }: Props) => {
    ...
    const currentLevel = getLevelData(stage, level, pitchCatchMockData);
    ...
    const updateDbWin = async () => {
        ...
        try {
            ...
            const res = await fetch(
                //@ts-ignore
                `${process.env.BASE_URL}/api/games/pitch-catch/${session?.user?.id}`,
                {
                    method: 'PUT',
                    headers: {
