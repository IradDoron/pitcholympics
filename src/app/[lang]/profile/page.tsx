    ...
    return (
        <div className='flex flex-col items-center justify-center h-full'>
            <Card color='primary' className='flex flex-col items-start p-4 gap-4' shadow='large' style={{ width: '400px' }}>
                <UserImage />
                <ProfileInfo label='Gender' inputType='select' options={['male', 'female']} onChange={handleGenderChange} />
                <ProfileInfo label='Email' inputType='text' isDisabled={true} value={session?.user?.email ?? ''} />
                <ProfileInfo label='Locale' inputType='select' options={['HE', 'EN']} onChange={handleLocaleChange} />
