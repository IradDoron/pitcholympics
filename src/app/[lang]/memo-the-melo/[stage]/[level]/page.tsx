...
import ButtonMelody from '@/components/buttonMelody';
...
const Page = ({ params }: Props) => {
    ...
    const checkUserGuess = (userGuess: number[], melody: number[]) => {
        ...
        if (currentNote < melody.length) { setCurrentNote(currentNote + 1) }
        setUserGuess([])
        return true
    }
        ...
        <>
            ...
            <div className='flex flex-row justify-center items-center gap-5 flex-wrap w-60 m-auto'>
                ...
                {(new Array(notesAmount)).fill(0).map((_, index) => {
                    ...
                    return <ButtonMelody key={index} noteId={index + 1} setUserGuess={setUserGuess} currentNote={currentNote} userGuess={userGuess} />
                })}
            </div>
            <button onClick={() => checkUserGuess(userGuess, melody)}>check user guess</button>
        </>
