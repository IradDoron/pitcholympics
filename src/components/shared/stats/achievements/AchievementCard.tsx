...
const AchievementCard = ({ user, achievement }: Props) => {
  ...
  const completedCount = userAchievement.criteriaStatus.reduce((count, curr) => count + (curr ? 1 : 0), 0);
  ...
  return (
    <div className="drop-shadow-xl p-2 bg-light-surface-tertiary dark:bg-dark-surface-tertiary flex-col text-center gap-3">
      ...
      <ProgressBar progress={completedCount} max={achievement.criteria.length} className='m-auto' />
      {show &&
        <ul className='m-1'>
          {achievement.criteria.map((criterion, index) => {
            return (
              <li key={achievement.id + index} className='text-sm font-inter font-bold text-light-surface-onTertiary dark:text-dark-surface-onTertiary'>
                <input type='checkbox' checked={userAchievement.criteriaStatus[index]} className='mr-2' disabled={true} />
      ...
      <div className='btn bg-transparent border-collapse' onClick={onClick}> {show ? 'O' : 'V'}</div>
    </div>
  )
}
